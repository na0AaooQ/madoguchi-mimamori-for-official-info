...
本番用`data/`には、地域4件、団体9件、案内先40件、確認根拠67件、5分野、案内カード13件、カード関連40件と日英localeを登録しています。5分野とその対象データをpublishedとしています。本番用`data/`とは別に、同じ管理契約を使うpublished架空fixtureを置き、preview用の日英`navigation.json`と画面を生成します。災害・出来事・履歴の本番用管理データは空で、専用の利用者向け表示は未実装です。
...
- 実在する地域、団体、案内先、確認根拠、5分野、13カード、40件のカード関連と日英locale
