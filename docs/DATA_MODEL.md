...
この文書は、第一版の管理データの正本と、各ファイルの責務・参照関係を定めます。各ファイルの確定フィールド仕様は[データフィールド定義](DATA_FIELDS.md)に記録します。工程3-1では、本番用の40データファイルと27 Schemaの枠組みを実装済みです。3つの`site.json`以外は空の`items`を持ち、site以外の正式なitem Schemaは未実装です。

実在する団体、URL、災害情報などは未登録です。全意味検証、公開対象抽出、公開成果物生成、画面も未実装であり、工程3-2以降で明確な架空データの最小縦切りから段階的に実装します。
...
工程3-1では、本番用の40データファイル、管理単位ごとの27 Schema、JSON Schema検証、配置検証、siteの最小意味検証を実装済みです。3つの`site.json`には初期設定と日英文面を持たせ、その他の37ファイルは空の`items`で開始しています。

次は工程3-2以降で実装・具体化します。

- 明確な架空データによる最小縦切り
- site以外の正式なitem Schema
- 37空ファイル間の参照を含む全意味検証
- 公開対象の抽出と日英成果物の生成
- 画面
- 改訂番号を増やす変更の詳細運用

実在する団体、URL、災害情報などは、架空データによる実装と検証の後の確認工程まで登録しません。
