...
5分野と、架空の国・都道府県、団体、日本語のみの案内先、公式性確認根拠、案内カード1件、カード関連1件と日英localeを、参照関係の検証用に`draft`で登録しています。工程3-2Aから工程3-2Bまでの最小縦切りは接続済みです。実在する団体、URL、災害情報などは未登録です。災害・出来事・履歴、公開対象抽出、公開成果物生成、画面は未実装です。
...
工程3-2Bでは5分野、架空カード1件、架空のカード関連1件を`draft`で登録しています。公開カードには構造上公開可能な`role: primary`の関連を最低1件必要とします。主案内先の最大件数と、現在日時による表示期間判定はこの工程で固定・実装しません。
