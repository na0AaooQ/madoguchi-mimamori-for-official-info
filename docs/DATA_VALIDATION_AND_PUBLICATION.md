...
この文書は、管理データを公開成果物へ変換する前後の検証責務、結果区分、停止条件、人による最終確認を定めます。各JSONの確定フィールド仕様は[データフィールド定義](DATA_FIELDS.md)を参照してください。工程3-2Aと工程3-2Bの管理Schema・意味検証に加え、通常カードの日英公開生成MVPを実装しています。実在情報と画面は未実装です。

品質管理基盤では、Node.js、`node:test`、Ajv、ESLint、Prettierと独自検証を採用しています。本番用の40データファイル、管理用27 Schema、工程3-2A・3-2Bの意味検証、公開用1 Schema、published架空fixture、公開対象抽出、生成、検証、鮮度比較を実装済みです。
...
公開生成MVPは、明示的な基準日による通常カードの期間判定、到達可能性抽出、allowlist構築、公開Schema、禁止キー、URL安全条件、productionライフサイクル、バイト一致を検証します。災害・出来事・履歴の意味検証、URL正規化後の重複、確認期限は未実装です。詳細は[公開データ契約](PUBLIC_DATA_CONTRACT.md)、[品質管理基盤](QUALITY_TOOLING.md)、[データSchema実装](DATA_SCHEMA_IMPLEMENTATION.md)を参照してください。
...
通常カードの公開対象抽出と`navigation.json`生成は実装済みです。災害・出来事関連の検証と公開生成は未実装です。
...
8. 公式情報の確認根拠を検証する
9. Errorがあれば停止する
10. `published`データだけを対象言語・基準日で抽出する
11. 公開カードから到達できる下流データだけを非正規化する
12. allowlist方式で内部専用項目を除外する
13. 日本語版・英語版の公開成果物を生成する
14. 公開Schema、禁止項目、URL安全条件を再検証する
15. Git管理中の成果物とバイト比較する
