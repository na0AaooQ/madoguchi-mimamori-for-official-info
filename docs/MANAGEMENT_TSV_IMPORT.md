...
各JSONは`schema_version`、引数で指定した`data_updated_on`、`items`を持ちます。`data_updated_on`は、そのTSV一式の管理データ更新日です。実行日を自動設定しないため、人が内容を確認したうえで`YYYY-MM-DD`形式の実在日を指定します。JSONは`JSON.stringify(value, null, 2)`を基に、リポジトリの既存Prettier設定で決定的に整形し、末尾改行を付けます。
...
| `show_in_official_source_list`                  | `true`、`false`、`TRUE`、`FALSE`をJSON booleanへ変換する |
...
値は勝手にtrim、正規化、置換、翻訳しません。完全一致する文字列「なし」はエラーです。配列のカンマ区切り文字列化、真偽値の`1`への読み替え、日付やIDの補正も行いません。GoogleスプレッドシートのチェックボックスがTSVへ出力する`TRUE`と`FALSE`は、それぞれJSON booleanの`true`と`false`へ変換します。セル内タブ・改行は第一版では使用できません。
...
`--check`は6 TSVを読み、構造・ヘッダー・行・型を検証して18候補JSONをメモリ上で生成します。その後、OSの一時ディレクトリへ現在の`data/`と`schemas/`をコピーし、対象18ファイルだけを候補へ置換して、既存の配置、Schema、意味、参照、locale、カード関連の全検証を実行します。公開カードでは、各公開言語に公開条件を満たす`role: primary`の関連があることも確認します。リポジトリの`data/`は変更しません。
