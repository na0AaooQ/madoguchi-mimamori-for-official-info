...
| 第一版公開         | 日本語・英語のproductionサイトを正式公開                                               |
| リポジトリ内成果物 | 実在管理データから、公開対象地域ごとの日英production公開データと静的サイトを生成・管理 |
| ホスティング構成   | GitHub Pages、カスタムドメイン、HTTPS有効                                              |
| デプロイ           | GitHub Pagesの手動workflowで公開。PRマージだけではproductionへ自動反映しない           |

本リポジトリには、設計文書、ローカル品質管理基盤、架空preview Web画面MVP、TSV変換基盤、実在管理データを整備しています。管理JSONには、地域、団体、案内先、確認根拠、案内カード、カード案内先関連等を登録しています。案内先と確認根拠は、人が確認した内容を管理JSONへ反映しています。

検証済みの公開対象データから、日英のproduction公開データと静的サイトを生成します。公開対象地域ごとの成果物を生成し、全国トップから各地域へ進む構成です。productionへの反映はPRマージ後にGitHub Pagesの手動workflowで行います。架空preview成果物は画面と生成基盤の確認専用としてproductionと分離しています。
...
全国トップは公開中の都道府県一覧を表示し、地域トップ以下の分野・団体一覧は対応する地域成果物から動的に生成します。リポジトリ内では、検証済みの公開対象地域・公開対象データからproduction成果物を生成します。

productionへの反映はPRマージだけでは自動化されず、GitHub Pagesの手動workflowによるデプロイが必要です。
...
`npm run check`でLint、書式、テスト、fixture、文書、管理データ、公開データ、preview・productionサイト、再現性をまとめて確認できます。テスト件数は今後変動するため、READMEへ固定値として記載しません。
...
- [開発作業標準フロー](docs/DEVELOPMENT_STANDARD_FLOW.md)

変更種別・適用判断・情報確認原則は「開発作業標準フロー 管理ルール」を正とし、Phase 0〜Phase 6の詳細手順は「開発作業標準フロー」を正とします。
...
検査が成功した後だけ、Core、日本語locale、英語localeの管理JSONへ書き込みます。
