...
| リポジトリ内成果物 | production静的サイト19ファイル。BL-019の本番デプロイ・公開後確認は未実施                      |
...
第一版で使用する「公的機関・防災全般」と「ライフライン」の2セクション、その対象管理データ、Core・日本語・英語のsiteはpublishedです。残る3セクションはdraftで、production成果物には含めません。現在のリポジトリでは、実在管理データから日英のproduction公開データと、正式サイトアイコン3点、production専用OGP画像1点を含む19ファイルの静的サイトを生成します。OGP画像とSNS共有メタデータの本番デプロイ・公開後確認は未実施です。GitHub Pagesのカスタムドメインで公開し、2026年8月4日の手動workflow実行 #2では18ファイルのbuildとdeployが成功しました。ルート言語選択、日英トップ、公開中分野、3カード、14案内先、一覧、プライバシーポリシー、問い合わせ導線、404、sitemap、CSS、文字サイズ、リンク、HTTPS、カスタムドメインを手動確認済みです。既存の架空preview成果物は、画面と生成基盤の確認専用として分離しています。
...
将来は、約17のまとまりを次の5分野に整理する設計です。現在のproductionでは「公的機関・防災全般」と「ライフライン」の2分野だけを公開しています。
...
分野ページはpublishedのセクションから動的に生成し、現在は「公的機関・防災全般」と「ライフライン」の2分野を公開対象としています。サイトの目的、利用上の注意、免責事項はトップページなどの`details`内で案内します。問い合わせは独立したサイト内ページを設けず、言語別の外部ポートフォリオ問い合わせページへ案内します。掲載方針と確認・更新履歴の独立HTMLページは将来検討であり、現在のproductionには存在しません。
