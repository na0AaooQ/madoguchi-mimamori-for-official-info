...
| リポジトリ内成果物 | BL-006-Bを反映したproduction静的サイト21ファイル。PRレビュー・マージ・手動デプロイ前          |
| ホスティング構成   | GitHub Pages、カスタムドメイン、HTTPS有効                                                     |
| デプロイ           | 手動workflow実行 #2でbuild・deploy成功、公開後の画面・リンク・アセット・HTTPS等を手動確認済み |

本リポジトリには、初期設計文書、ローカル品質管理基盤、架空preview Web画面MVP、TSV変換基盤、第一版の実在管理データを整備しています。管理JSONには、地域3件、団体4件、案内先16件、確認根拠29件、案内カード4件、カード案内先関連16件を登録しています。案内先と確認根拠は、人が確認した内容を管理JSONへ反映しています。

リポジトリ内では「公的機関・防災全般」「ライフライン」「道路・交通」の3セクションと対象データをpublishedとし、実在管理データから日英のproduction公開データと、正式サイトアイコン3点、production専用OGP画像1点を含む21ファイルの静的サイトを生成します。「命・安全・医療」と「支援・復旧」はdraftです。GitHub Pagesで現在公開中なのは2026年8月4日に手動workflow実行 #2でdeployした2分野・3カード・14案内先であり、BL-006-Bの反映にはPRレビュー・マージ後の手動デプロイと公開後確認が必要です。既存の架空preview成果物は、画面と生成基盤の確認専用として分離しています。
...
公式性の確認根拠は管理データと内部検証に使用し、現在のproduction HTMLには根拠の説明や根拠URLを表示しません。確認・更新履歴の利用者向け公開や、期間限定の災害別公式案内は将来検討です。
...
将来は、約17のまとまりを次の5分野に整理する設計です。リポジトリ内のproduction成果物では「公的機関・防災全般」「ライフライン」「道路・交通」の3分野を公開対象としています。GitHub Pagesへの公開反映は手動デプロイ後です。
...
支援・復旧は将来の公開画面上で一つのまとまりとして扱う設計です。第一版以降の掲載候補は[情報掲載方針](docs/INFORMATION_LISTING_POLICY.md)に記載しています。将来拡張時の最終的なカード数は固定しません。
...
分野ページはpublishedのセクションから動的に生成し、リポジトリ内では「公的機関・防災全般」「ライフライン」「道路・交通」の3分野を公開対象としています。GitHub Pagesで現在公開中の2分野へ反映するには、PRのマージ後に手動デプロイが必要です。サイトの目的、利用上の注意、免責事項はトップページなどの`details`内で案内します。問い合わせは独立したサイト内ページを設けず、言語別の外部ポートフォリオ問い合わせページへ案内します。掲載方針と確認・更新履歴の独立HTMLページは将来検討であり、現在のproductionには存在しません。
...
Node.js `24.18.0`とnpm `11.16.0`を対象環境として固定しています。依存関係を`npm ci`で準備した後、`npm run validate:data`で本番用データ基盤を検証でき、`npm run check`でLint、書式、552件のテスト、fixture、文書、管理データ、公開データ、preview・productionサイト、再現性をまとめて確認できます。2026年8月4日の第一版公開時点で`npm run check`は成功しています。詳しくは[品質管理基盤](docs/QUALITY_TOOLING.md)を参照してください。
...
ローカルでは<http://127.0.0.1:4173/>を開きます。production成果物は`dist/site/production/`にあり、ルート言語選択、日英トップ、公開対象3分野、全団体・案内先一覧、プライバシーポリシー、`404.html`、`sitemap.xml`を含みます。GitHub Pagesへの反映はまだ行わず、正式base URL、手動workflow、公開後確認、再デプロイは[GitHub Pages手動デプロイ](docs/GITHUB_PAGES_DEPLOYMENT.md)を参照してください。
