...
現在登録されている地域・団体・案内先・確認根拠は、データ構造と参照関係を確認するための架空データです。実在情報の掲載・更新・削除手順は、公開用データ生成と画面仕様の確定後に整備します。それまでは実在する団体、URL、災害情報を登録しないでください。
...
工程3-2Aでは、地域・団体・案内先・確認根拠のcoreとlocaleに対応する8 Schemaを正式化し、12データファイルに下書きの架空データを登録しています。配置検証、site検証、工程3-2Aの参照・locale・公開状態・公式性確認根拠の意味検証を実装済みです。工程3-2Bの分野・カード・関連データ、公開対象抽出、公開生成処理、画面は未実装です。詳しくは[データSchema実装](docs/DATA_SCHEMA_IMPLEMENTATION.md)を参照してください。ホスティング方式、AWS構成、ドメイン、URLパス、英語版を`/en/`へ配置するか、CI/CD、JavaScriptの具体的範囲、アクセス解析、CDN・サーバーログ、Cookie、ライト・ダークモードの具体的実装、全国展開は後続工程で検討します。
