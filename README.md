...
| リポジトリ内成果物 | BL-006-Bを反映したproduction静的サイト21ファイル。BL-006-Bは本番反映・公開後確認済み |
| ホスティング構成   | GitHub Pages、カスタムドメイン、HTTPS有効                                            |
| デプロイ           | GitHub Pagesの手動workflowで公開。BL-006-Bは2026年8月7日に本番反映・公開後確認済み   |
...
リポジトリ内では「公的機関・防災全般」「ライフライン」「道路・交通」の3セクションと対象データをpublishedとし、実在管理データから日英のproduction公開データと、正式サイトアイコン3点、production専用OGP画像1点を含む21ファイルの静的サイトを生成します。「命・安全・医療」と「支援・復旧」はdraftです。BL-006-B「道路・交通」は2026年8月7日にGitHub Pagesへ手動デプロイし、公開後の画面・リンク・一覧・404・sitemap等を確認済みです。現在のproductionでは3分野、4カード、16案内先を公開しています。既存の架空preview成果物は、画面と生成基盤の確認専用として分離しています。
...
将来は、約17のまとまりを次の5分野に整理する設計です。現在のproductionでは「公的機関・防災全般」「ライフライン」「道路・交通」の3分野を公開しています。
...
分野ページはpublishedのセクションから動的に生成し、現在は「公的機関・防災全般」「ライフライン」「道路・交通」の3分野を公開対象としています。
...
`npm run check`でLint、書式、610件のテスト、fixture、文書、管理データ、公開データ、preview・productionサイト、再現性をまとめて確認できます。
2026年8月4日の第一版公開時、および2026年8月7日のBL-006-B公開後の確認でも`npm run check`は成功しています。
