...
| リポジトリ内成果物 | BL-006-C第一公開単位まで反映したproduction静的サイト23ファイル。2026年8月8日に本番反映・公開後確認済み |
| ホスティング構成   | GitHub Pages、カスタムドメイン、HTTPS有効                                                              |
| デプロイ           | GitHub Pagesの手動workflowで公開。BL-006-C第一公開単位は2026年8月8日に本番反映・公開後確認済み         |

本リポジトリには、初期設計文書、ローカル品質管理基盤、架空preview Web画面MVP、TSV変換基盤、第一版の実在管理データを整備しています。管理JSONには、地域3件、団体4件、案内先18件、確認根拠31件、案内カード5件、カード案内先関連18件を登録しています。案内先と確認根拠は、人が確認した内容を管理JSONへ反映しています。

リポジトリ内では「公的機関・防災全般」「ライフライン」「道路・交通」「支援・復旧」の4セクションと対象データをpublishedとし、実在管理データから日英のproduction公開データと、正式サイトアイコン3点、production専用OGP画像1点を含む23ファイルの静的サイトを生成します。「命・安全・医療」はdraftです。BL-006-C「支援・復旧」第一公開単位は2026年8月8日にGitHub Pagesへ手動デプロイし、公開後の画面・リンク・一覧・404・sitemap等を確認済みです。現在のproductionでは4分野、5カード、18案内先を公開しています。既存の架空preview成果物は、画面と生成基盤の確認専用として分離しています。
...
「支援・復旧」は2026年8月8日に第一公開単位を公開済みです。「命・安全・医療」は引き続きdraftです。第一版以降の掲載候補は[情報掲載方針](docs/INFORMATION_LISTING_POLICY.md)に記載しています。今後のカード数は固定しません。
...
`npm run check`でLint、書式、611件のテスト、fixture、文書、管理データ、公開データ、preview・productionサイト、再現性をまとめて確認できます。
2026年8月4日の第一版公開時、2026年8月7日のBL-006-B公開後、および2026年8月8日のBL-006-C第一公開単位公開後の確認でも`npm run check`は成功しています。
