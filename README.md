...
本リポジトリには、初期設計文書とローカル品質管理基盤の土台を整備しています。掲載候補は設計上の対象であり、本番データ・画面として実装済みまたは公開済みではありません。
...
テスト専用の小さなJSON Schema、最小の意味検証、文書検証は品質管理基盤として実装しています。本番用JSON、全種類のJSON Schema、全意味検証、公開生成処理は未実装です。ホスティング方式、AWS構成、ドメイン、URLパス、英語版を`/en/`へ配置するか、CI/CD、JavaScriptの具体的範囲、アクセス解析、CDN・サーバーログ、Cookie、ライト・ダークモードの具体的実装、全国展開は後続工程で検討します。

## 品質管理

Node.js `24.18.0`とnpm `11.16.0`を対象環境として固定しています。依存関係を`npm ci`で準備した後、`npm run check`でLint、書式、テスト、fixture、文書固有検証をまとめて実行できます。詳しくは[品質管理基盤](docs/QUALITY_TOOLING.md)を参照してください。
